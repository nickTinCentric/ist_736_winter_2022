...
####adding my code for excesize
# Take the program we just walked through and modify it to create an output file called 'under-million.csv' 
# and change the code to only select players who make under $310,000.
#create file name
outfile3 = infile.replace('tsv', '') + 'under-million.csv'

#loop through and write to output file 3
with open(outfile3, 'w', newline='') as csvfileout:
  #create writer object
  writer = csv.writer(csvfileout, delimiter=',', quoting=csv.QUOTE_MINIMAL)

  #open writer and write header row
  writer.writerow(['Player', 'Team', 'Salary'])
  
  #loop through data and write each line to file
  for player in playersList:
    if(player['sal'] < 310000):
      writer.writerow([player['name'], player['team'], player['sal']])
